...
const Quizer: React.FC<QuizerProps> = ({ quizerJson }) => {
  const [currentQuestion, setCurrentQuestion] = React.useState<number>(0)
  const [choose, setChoose] = React.useState<string>("")
  const [chooseGroup, setChooseGroup] = React.useState<string[]>([])
  const [pattern, setPattern] = React.useState<Array<{ question: number; choose: string | string[]; correctAnswer: string; point: string }>>([])

  const [showResult, setShowResult] = React.useState<boolean>(false)
  
  const handleNextQuestion = (index: number, answerType: string) => {
    if(answerType === "text" || answerType === "radio") {
      if(choose !== "" && choose !== null) {
        ...
        setChoose("") // Reset para nova pergunta
      ...
      setChooseGroup([]) // Reset para nova pergunta
      setCurrentQuestion(currentQuestion + 1)
    }
  }

  const handleChoose = (value: string, answerType: string) => {
  ...
  if(currentQuestion + 1 > quizerJson.length) {
      ...
      return (
        <div className="border border-slate-800 p-10 shadow-lg rounded-xl bg-slate-900">
          {pattern.map((item, index) => {
            item.choose === item.correctAnswer ? points += parseInt(item.point) : points
            return (
              <div className="text-slate-50" key={item.question}>
    ...
    return (
      <div className="flex flex-col gap-10 justify-center items-center p-10">
        <div className="flex flex-col justify-center items-center min-w-[600px] border border-slate-800 p-10 rounded-xl text-slate-300 bg-slate-900">
          <IconCircleCheck size={128} className="text-teal-600" />
          <h2 className="text-slate-100 text-3xl mb-5">Parabéns</h2>
          <p className="text-slate-400 mb-10">Você chegou ao fim do QUIZ DEMO. <br /> Clique em finalizar para ver o resultado.</p>
          <button
  ...
  return (
    <div className="flex flex-col justify-center items-center p-10">
      <div className="min-w-[600px] border border-slate-800 p-10 rounded-xl text-slate-300 shadow-lg bg-slate-900">
        <div className="mb-10">
...

          {
            ...
            : quizerJson[currentQuestion].answers.length > 0 &&
              quizerJson[currentQuestion].answers.map((item, index) => (
                <label
                  key={item}
                ...
                >
                  {quizerJson[currentQuestion].answerType === "checkbox" ? (
                    ...
                    <input
                      ...
                      value={index}
                      checked={choose === index.toString()}
                      onChange={(event) => handleChoose(event.target.value, quizerJson[currentQuestion].answerType)}
                    />
                  )}

                  {choose === index.toString() && <IconCircleCheck size={18} className="mr-1" />}
                  {item}
                </label>
              ))
          }
